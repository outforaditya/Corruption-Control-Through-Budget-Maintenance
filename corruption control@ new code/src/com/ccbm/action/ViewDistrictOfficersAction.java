...
public class ViewDistrictOfficersAction extends HttpServlet {
	...
	try{
		...
		try{
			...
			vdo=new RegisterMgrDelegate().viewDistOfficers();
		...
		if(!vdo.isEmpty()){
			...
			request.setAttribute("status", UtilConstants._DIST_OFFICERS_DETAILS);
			
			request.setAttribute("distofficers",vdo);
			session.setAttribute("deptofficers", vdo);
			
			path=UtilConstants._VIEW_DIST_FOR_DEPTOFFICERS;
	
		}
		else
		{
			
			request.setAttribute("status", UtilConstants._NO_DIST_OFFICERS_DETAILS);
			path=UtilConstants. _VIEW_DIST_FOR_DEPTOFFICERS;
